...
const MESSAGES: &[(&str, &str, &str)] = &[
    ...
    ("brand.subtitle", "VPN workspace", "Central de VPNs"),
    (
        "brand.subtitleMulti",
    ...
    ("ops.quickActions", "Quick actions", "Ações rápidas"),
    ("ops.workspace", "CONNECTION WORKSPACE", "CENTRAL DE CONEXÕES"),
    (
        "ops.workspaceSummary",
        "{total} profiles · {active} active now",
        "{total} perfis · {active} ativos agora",
    ),
    ("ops.totalProfiles", "total profiles", "perfis no total"),
    ("ops.connectedNow", "connected", "conectadas"),
    ...
    (
        "ops.autoRelinkHint",
        "Restore a tunnel automatically after an unexpected drop.",
        "Restaura o túnel automaticamente após uma queda inesperada.",
    ),
    (
        "ops.startWithLinuxHint",
        "Keep TunnelYard available from the system tray after login.",
        "Mantém o TunnelYard disponível na bandeja após entrar no sistema.",
    ),
    ...
    ("profiles.noMatchTitle", "No profile found", "Nenhum perfil encontrado"),
    (
        "profiles.noMatch",
        "No profile matches “{query}”.",
        "Nenhum perfil corresponde a “{query}”.",
    ),
    ...
    (
        "form.editorHint",
        "Connection details are saved in an openfortivpn .conf profile.",
        "Os dados são salvos em um perfil .conf do openfortivpn.",
    ),
    ...
    ("form.healthHint", "Optional note stored in the .conf. Linux uses openfortivpn's own tunnel-up signal.", "Nota opcional no .conf. No Linux o openfortivpn já sinaliza quando o túnel sobe."),
    ...
    ("form.legacyTunnelHint", "Kept as a .conf comment for gateways that need openfortivpn's legacy TLS hand-off.", "Guardado como comentário no .conf para gateways que precisam da troca TLS antiga do openfortivpn."),
    ...
    ("update.aptHint", "Download and install the Linux package. PolicyKit may ask for authorization.", "Baixe e instale o pacote Linux. O PolicyKit pode pedir autorização."),
