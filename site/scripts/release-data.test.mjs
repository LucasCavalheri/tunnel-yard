...
test("release fetch authenticates with the optional build token", async () => {
  ...
  const release = await fetchLatestRelease(async (url, options) => {
    ...
    return new Response(
      JSON.stringify({
        tag_name: "v3.0.8",
        html_url: `${repoUrl}/releases/tag/v3.0.8`,
        assets: [{ name: "installer", browser_download_url: `${repoUrl}/releases/download/v3.0.8/installer` }],
      }),
      { status: 200 },
    );
  ...
  assert.equal(release.tag_name, "v3.0.8");
});
