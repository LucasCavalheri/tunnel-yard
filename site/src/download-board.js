...
function refresh(board) {
  ...
  if (status) {
    ...
    status.textContent = distro
      ? template.replace("{distro}", name).replace("{package}", pack)
      : board.dataset.pickHint || "";
  }
}
...
export function bindDownloadBoard(root = document) {
  root.querySelectorAll("[data-download-board]").forEach((board) => {
    ...
    const defaultArch = arches.find((chip) => chip.dataset.arch === "x64") || arches[0];
    if (defaultArch) setRadio(arches, defaultArch);
    refresh(board);
  });
}
